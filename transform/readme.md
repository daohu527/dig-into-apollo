关于transform模块开始一直不知道是干啥的，一直看到一个"/tf"的TOPIC，开始还以为是tensorflow的缩写，想着是不是和神经网络有关系，后来才知道tf是transform的缩写，主要的用途是进行坐标转换，原型即是大名鼎鼎的ros/tf2库。那么为什么要进行坐标转换呢？  
...
## Buffer
Buffer实际上提供了一个工具类给其他模块，它的主要作用是？？？，我们先看BufferInterface的实现：  
```
