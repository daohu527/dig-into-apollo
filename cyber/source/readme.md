...
DataVisitor（消息访问器）是一个辅助的类，**每一个Reader创建的时候对应一个DataVisitor，通过在DataVisitor中注册Notify（通知），注册对应的Buffer，和对应的数据处理回调函数**，这样在DataDispather的时候会通知对应的DataVisitor去处理回调函数。  
...
tick 为2ms
```
![timing_wheel_conf](../img/timing_wheel_conf.jpg)  
...
```
1. 从上述过程可以看出Cyber的时间轮只有一个线程去执行任务，当任务较多的时候，会出现调度不过来的情况，因此需要注意不要增加过多的TimerComponent，并且TimerComponent中消息的处理要尽量快的执行，不能在Timer模块中引入阻塞的操作。 
